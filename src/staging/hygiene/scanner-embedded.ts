/**
 * Embedded instruction pattern scanner.
 *
 * Scans content for prompt injection patterns: ignore previous
 * instructions, role reassignment, chat template delimiters,
 * and system prompt overrides.
 *
 * @module staging/hygiene/scanner-embedded
 */
...
/**
 * Scan content for embedded instruction injection patterns.
 *
 * Fetches all patterns in the 'embedded-instructions' category
 * from the registry and applies each against the content.
 *
 * @param content - Text content to scan
 * @returns Array of findings (empty if clean)
 */
export function scanEmbeddedInstructions(content: string): HygieneFinding[] {
